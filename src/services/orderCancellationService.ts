    ...
    await Promise.all([
      ...
      emailService.sendEmail({
        ...
        html: createEmailTemplate(
          { title: "Order Cancelled", headerType: "error", headerText: "🔁 Order Cancelled" },
          `
  ...
  private static async notifySellerOfBuyerCancellation(
    ...
    await Promise.all([
      ...
      emailService.sendEmail({
        ...
        html: createEmailTemplate(
          { title: "Order Cancelled by Buyer", headerType: "error", headerText: "🔁 Buyer Cancelled Order" },
          `
  ...
  private static async notifyBuyerOfSellerDecline(
    ...
    await Promise.all([
      ...
      emailService.sendEmail({
        ...
        html: createEmailTemplate(
          { title: "Order Declined", headerType: "error", headerText: "❌ Order Declined" },
          `
          <p>Hi ${order.buyer.name},</p>
  ...
  private static async notifySellerMissedPickup(order: Order): Promise<void> {
    ...
    await Promise.all([
      ...
      emailService.sendEmail({
        ...
        html: createEmailTemplate(
          { title: "Missed Courier Pickup", headerType: "warning", headerText: "⚠️ Missed Courier Pickup" },
          `
          <p>Hi ${order.seller.name},</p>
  ...
  private static async notifyBuyerOfPickupDelay(order: Order): Promise<void> {
    ...
    await Promise.all([
      ...
      emailService.sendEmail({
        ...
        html: createEmailTemplate(
          { title: "Delivery Update", headerType: "warning", headerText: "📦 Pickup Delayed" },
          `
          <p>Hi ${order.buyer.name},</p>
    ...
    if (order.buyer) {
      await Promise.all([
        ...
        emailService.sendEmail({
          ...
          html: createEmailTemplate(
            { title: "Delivery Rescheduled", headerText: "🔁 Delivery Rescheduled" },
            `
            <p>Hi ${order.buyer.name},</p>
  ...
  private static async notifyBuyerOfSellerCancellation(
    ...
    await Promise.all([
      ...
      emailService.sendEmail({
        ...
        html: createEmailTemplate(
          { title: "Order Cancelled", headerType: "error", headerText: "🛑 Order Cancelled by Seller" },
          `
          <p>Hi ${order.buyer.name},</p>
