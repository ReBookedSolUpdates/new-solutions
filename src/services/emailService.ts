...
import { getTemplateById } from "@/utils/emailTemplateRegistry";
...
export const EMAIL_TEMPLATES = {
  ...
  PAYMENT_CONFIRMATION_RECEIPT: "payment-confirmation-receipt",
  ORDER_COMMITMENT_CONFIRMED: "order-commitment-confirmed",
  ORDER_CANCELLED: "order-cancelled",
  ORDER_DELIVERED_BUYER: "order-delivered-buyer",
...
class EmailService {
  ...
  private generateEmailFromTemplate(templateName: EmailTemplateName, data: Record<string, any>): { html: string; text: string } {
    ...
    if (template) {
      try {
        const generated = template.generator(data);
        return {
          html: generated.html,
          text: generated.text || ""
        };
      } catch (error) {
        console.error(`Error generating email from template ${templateName}:`, error);
      }
    }

    // Fallback to existing logic for legacy templates not yet in registry
    switch (templateName) {
...
support@rebookedsolutions.co.za`;
  ...
  async sendOrderCancelled(
    ...
    return this.sendTemplateEmail(to, EMAIL_TEMPLATES.ORDER_CANCELLED, data);
  }

  async sendOrderDeliveredBuyer(
    to: string,
    data: {
      buyerName: string;
      orderId: string;
    ...
    return this.sendTemplateEmail(to, EMAIL_TEMPLATES.ORDER_DELIVERED_BUYER, data);
  }
  ...
  async sendChatNotification(
    ...
    return this.sendTemplateEmail(to, EMAIL_TEMPLATES.CHAT_NOTIFICATION, data);
  }

  async sendOrderCommitmentConfirmed(
    to: string,
    data: {
      recipientName: string;
      role: 'buyer' | 'seller';
    ...
    return this.sendTemplateEmail(to, EMAIL_TEMPLATES.ORDER_COMMITMENT_CONFIRMED, data);
  }
  ...
  async sendPaymentReceipt(
    ...
    return this.sendTemplateEmail(to, EMAIL_TEMPLATES.PAYMENT_CONFIRMATION_RECEIPT, data);
  }
